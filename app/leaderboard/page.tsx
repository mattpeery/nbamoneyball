...
export default async function LeaderboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F4F5F6] pb-6">
      ...
      {pinnedRow ? (
        <PinnedEntryCard row={pinnedRow} rank={pinnedIndex + 1} isPlayoff={isPlayoff} editable={editable} groupId={PUBLIC_GROUP_ID} />
      ) : (
        editable && (
          <div className="mx-4 mt-4 bg-white border border-[#DADFE3] rounded-2xl p-4 text-center">
            <p className="text-[13px] text-[#6B7280] mb-3">You haven&apos;t built a roster yet.</p>
            <Link
              href={draftPathFor(PUBLIC_GROUP_ID)}
