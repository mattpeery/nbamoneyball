...
export async function sendEmail({ to, subject, html }: { to: string; subject: string; html: string }): Promise<void> {
  ...
  if (!apiKey) {
    console.log(`[email] RESEND_API_KEY not set - skipping "${subject}" to ${to}. Body:\n${html}`);
    return;
  }
  try {
...
export function passwordResetEmail({ resetUrl }: { resetUrl: string }): string {
  return shell(`
    <h1 style="font-size:22px;font-weight:800;margin:16px 0 4px;">Reset your password</h1>
    <p style="font-size:14px;color:#6B7280;margin:0 0 20px;">Tap the button below to set a new password. This link expires in 1 hour.</p>
    <a href="${resetUrl}" style="display:inline-block;background:#16A34A;color:#fff;text-decoration:none;font-weight:700;font-size:14px;padding:12px 24px;border-radius:12px;">Reset password</a>
    <p style="font-size:12px;color:#9AA0A6;margin:20px 0 0;">If you didn't request this, you can ignore this email.</p>
  `);
}
...
export function deadlineReminderEmail({
  ...
  return shell(`
    <h1 style="font-size:22px;font-weight:800;margin:16px 0 4px;">Your picks lock tomorrow!</h1>
    <p style="font-size:14px;color:#6B7280;margin:0 0 16px;">${entryName}, picks lock for the season on ${deadlineLabel}. Here's what you've got:</p>
    <div style="margin-bottom:16px;">${teamsHtml}</div>
    <a href="${leaderboardUrl}" style="display:inline-block;background:#16A34A;color:#fff;text-decoration:none;font-weight:700;font-size:14px;padding:12px 24px;border-radius:12px;">Edit your picks</a>
  `);
}
